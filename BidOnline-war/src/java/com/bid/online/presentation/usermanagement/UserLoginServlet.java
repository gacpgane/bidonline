    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user != null) {
            if (user.getPassword().equals(password)) {
                ...
                session.setAttribute(StringUtils.USER, user);
                session.setAttribute(StringUtils.AUTH_FLAG, true);
                authenticated = true;
            }
        }

        if (!authenticated) {
            request.setAttribute(StringUtils.MESSAGE_ERROR_KEY, UIMessages.LOGIN_FAILURE);
